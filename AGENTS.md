# AGENTS.md — copier_dev_project

House rules: [`AGENTS.canon.md`](AGENTS.canon.md) (profiles `00-core`,
`10-python`) — generated by `~/code/_templates/agents_canon`, never edited here.
This file carries only what is true in *this* repository. The incidents behind
the rules: [`LOGBOOK.md`](LOGBOOK.md).

`just check` is the gate here: it verifies this repo's canon copy and then
renders the template for real. This repository stands on rung `gate` of the
hook ladder — `commit-msg` and `pre-commit` run it on every commit.

## What this repo is

The **generic base scaffold** for every code project: Python 3.13 with uv, just,
ruff, mypy, pytest, the documentation canon, and an optional publication layer
(Typst tooling: `freeze.py`, `prepare_assets.py`, `export_symbols.py`,
`export-pdf`).

It is deliberately the *base only*. Since v3.0.0 everything report- or
thesis-specific lives in its own world repository — `gutachten_bausteine` and
`thesis_bausteine` — which is applied as a second copier layer on top.
→ `LOGBOOK.md`, 2026-08-17.

Two sibling scaffolds, not to be confused: `copier_project` builds the **data**
side under `~/data/`, and `agents_canon` supplies the house rules this template
vendors.

## Keep it generic — the rule the v3.0.0 cut bought

**Nothing that only one kind of project needs goes in here.** No report chapters,
no thesis structure, no domain schema, no personal data. If a change would be
wrong for a plain data-analysis project, it belongs in an overlay.

Shared ground *does* belong here: `scripts/check.py|clean|common|pipeline|setup`,
the test and lint scaffolding, `schemas/columns.py` as the symbol and unit SSOT
both worlds hang off, and `publication/lib/data.typ`.

## The two anchor points for overlays

They are formulated generically on purpose, so the base never has to know which
overlay exists:

1. `import? 'project.just'` in `justfile.jinja` — the `?` keeps it a no-op when
   no overlay is applied. An overlay ships its recipes as `project.just`.
2. `AGENTS.md.jinja` points at `docs/*/AGENTS.md` — an overlay ships its binding
   text rules there, and the canon's nested-AGENTS rule then applies to them.
3. `import? 'local.just'`, below the overlay's import: the third layer, the
   project's own recipes, so that neither the template nor an overlay ever has
   to merge around what one repository invented for itself.

An overlay also brings its own copier answers file, so both layers update
independently.

## Canon in every generated project

**This template no longer ships `AGENTS.canon.md` or `.agents-canon`.** A
generated project fetches them itself — `just setup` runs `canon-pull`, and so
can you at any time.

**Why:** shipping the canon meant this template carried a second copy with *its
own* profile list, and every later `copier update` offered that copy to the
project — the house rule that nothing writes into another repository, applied to
the template itself. → `LOGBOOK.md`, 2026-08-18.

The `canon_profiles` answer **only bootstraps the first pull**. Once
`.agents-canon` exists it is authoritative, so a project can add a profile by
editing that file and running `canon-pull`; a stale copier answer can never
narrow it back. The recipe says so at the point of use.

A project between generation and its first pull is a normal state: the gate's
canon stage returns early when `AGENTS.canon.md` is absent.

## `project_kind` — what the repository produces

**Ask the distinction once, derive the flags from it.** `code`, `report` or
`thesis`; the two writing values name the overlay that follows
(`gutachten_bausteine`, `thesis_bausteine`).

**Why:** the code-vs-text split decides several flags, and asking it one flag at
a time invites a project that answers itself inconsistently — a Gutachten that
carries VISION.md because nobody thought about that one question.
→ `LOGBOOK.md`, 2026-08-21.

Today only `include_vision_roadmap` derives from it: planning files are a
software project's, a report's goal stands in the commission and its date in the
deadline. The canon lists both files as optional, so a repository without them
is in order.

**Derive the default, not the question.** A `when:` that suppresses a question
costs files: a suppressed question does not fall back to its default, copier
**drops the recorded answer**, so a `true` already written in an answers file
becomes a deletion on the next update. → `LOGBOOK.md`, 2026-09-03.

The derived default carries the whole job on its own. A Gutachten got VISION.md
because the question defaulted to **yes**, not because it was asked; with the
default at `false` nobody has to remember a "no", and the objection that "a 'no'
that has to be remembered every time is a 'yes' waiting to happen" no longer
applies.

**Why it generalises:** a `when:` clause is not a default, it is an assertion
that the answer can never legitimately differ. Reserve it for questions where
that is true.

`git_hooks` below derives its default the same way, and stays a real question
for the same reason: a code repository without a gate is a deliberate choice
often enough to be worth the keystroke.

## The hook ladder

**The rules live in the canon** — `AGENTS.canon.md`, section "Git hooks": what
each rung may run, why no test belongs in `pre-commit`, and why a hook may never
fail for a reason the commit did not cause. This file only says which rung a
project kind gets.

| Rung | Hooks | Default for |
|---|---|---|
| `none` | — | nothing; a sandbox or a throwaway |
| `message` | `commit-msg` | **report**, **thesis** |
| `gate` | + `pre-commit` → `just check` | — |
| `full` | + `pre-push` → `just check test` | **code** |

A writing repository gets `message` rather than nothing because the commit
convention costs milliseconds and judges only the line just typed. It stops
short of `gate` because there the deliverable is a PDF and the gate would check
Python nobody touched: what protects the document is `just pub-watch` while
writing and `just deliver` / `just submit`, which refuse a dirty tree and
compile fresh. A commit there is a save point, not a publication, and blocking
a save point over a transient state teaches exactly one habit, `--no-verify` —
which voids the gate everywhere. That has already happened once in `ba_ks`.

`gate` exists for the repositories in between, this one included: a code project
whose `ci` is only `check`, with no test tiers to add.

The hooks are **versioned** in `scripts/git-hooks` and wired in with
`core.hooksPath` — never the `pre-commit` framework. The framework stashes
unstaged work before running, so its verdict describes a tree that never existed;
the versioned hooks judge the working tree as it is. That is also why a generated
project carries no `pre-commit` dependency at all: each hook is a few lines of
bash, and only the last one calls a `just` verb.

## Every test tier ships a test

`tests/unit/`, `tests/integration/` and `tests/system/` each carry a
`test_smoke.py`, and `check-render` asserts that all three arrive in a generated
project. Replace them with real tests; never leave a tier empty.

**Why:** a tier directory that holds only a `.gitkeep` does not arrive at all —
`copier.yml` excludes `**/.gitkeep` from the render — and pytest answers a
*missing* directory with exit 4, a usage error that the recipes'
`|| test $? -eq 5` does not catch. A placeholder the render drops is not a
placeholder. → `LOGBOOK.md`, 2026-08-26.

## Verifying a change

The template has no tests, because the thing that can break is the render. So
`just check-render` *is* the test: it generates a throwaway project and asserts
that the shipped canon pin still matches its file, that the generated `justfile`
parses, and that the generated Python compiles. Each of those has been shown to
fail on a deliberately broken template.

It renders with `--vcs-ref HEAD` on a local path, which copier resolves to the
**dirty working tree** — so the gate judges what you are about to commit, not
what you last committed. The module name it passes is deliberately long and
awkward, because identifier length and underscores are where the Jinja paths
break first.

`just probe` does the same but keeps the directory, for looking at the result.
For a change you want to be sure about, run the generated project's own gate on
top: `cd /tmp/probe-dev-project && just check`.

A template that renders in your head is not a template that renders.

## Commit default

**Execute** — atomic commits are expected once asked for. Pushing needs its own
word. Tag releases annotated (`git tag -a`); consumers pin `_commit` from the
tag, and `git push --follow-tags` skips lightweight ones.
