...
1. Ensure the user has provided a target scope (e.g., `/test-audit sensors` or `/test-audit all`). If not, ask them which module they want to audit.
2. Read the testing conventions:
   - `docs/development/testing.md` (full testing guide including anti-patterns and gatekeeper rules)
   - `AGENTS.md` §4 — Testing Rules
...
6. Evaluate the test code against these rules:
   ...
   - **Anti-Patterns (§6.1):** Flag Existence/Import Tests, Trivial Equality, Call-Chain Mirroring, Mock-Heavy Verification.
   - **Delete vs. Refactor (§6.2):** Recommend DELETE for valueless tests, REFACTOR for brittle but valuable ones.
   - **Layer Rules (§4):**
